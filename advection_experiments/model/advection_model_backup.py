...
class AdvectionModel:
    def __init__(
        ...
        #self.J_var = fd.derivative(self.F_weak, self.u, bcs=self.bcs)
        ...
        # Solver for G
        G_problem = self.G_problem = fd.NonlinearVariationalProblem(self.R_G, G, bcs=bcs)
        self.G_solver = self.G_solver = fd.NonlinearVariationalSolver(G_problem)
    ...
    def eval_loss(self, pde_form):
        ...
        if pde_form == 'weak':
            ...
            # If we have advection, then we need to compute the funciton G(u)
            # to evaluate the variational principle
            self.G_solver.solve()
            R = fd.assemble(self.I_u, bcs=self.bcs)
            du = compute_gradient(R, Control(self.u))
            self.du.assign(du)
