...
inlet = LEFT
v_inlet = fd.Constant((2.0e-6, 2.0e-6))
cIn = 2.0                   # injection conc

# problem parameters
verbose = False
freq_res = 10
dt = 5000   # time steps size (initial)
sim_time = 2e6     # simulation time

ptimes = np.linspace(0., sim_time, 25)
...
# 2) Define mesh
print('* define mesh')
mesh = fd.RectangleMesh(nx, ny, Lx, Ly, quadrilateral=quad_mesh)
...
# Source term
fc = fd.Function(X, name='source')
coords = mesh.coordinates.dat.data[:]
xs = coords[:,0]
ys = coords[:,1]

indexes0 = np.logical_and(xs > 0.4, xs < 0.6)
indexes1 = np.logical_and(ys > 0.4, ys < 0.6)
indexes = np.logical_and(indexes0, indexes1)
fc.dat.data[indexes] = 1.

# 3.3) set boundary conditions
t_bc0 = fd.DirichletBC(X, cIn, LEFT)
t_bc1 = fd.DirichletBC(X, cIn, BOTTOM)
